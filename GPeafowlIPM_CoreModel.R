...
JuM_BN <- c(13,17,28,29,8,8,20,9,10,36,11,19,11,0)   #Male juveniel count
...
## Nimble code for run the whole model
  ...
  GP.IPMcode <- nimbleCode({
    ...
    for (t in 1:ny){
      ...
      # ChF_NB[t] ~ dpois(p[t] * NNon[1,t])
      # Br_NB[t] ~ dpois(p[t] * NNon[4,t])
      # 
      # 
      # #Observation Model in Breeding
      # JuF_BN[t] ~ dpois(p[t] * NBreed[2,t])
      # Br_BN[t] ~ dpois(p[t] * NBreed[4,t])
