...
make.GPprojMatrix <- function(pRep, mean.CS, S_C, gamma, sF_NB, sF_BN, sM_NB, sM_BN,
  ...
  if(seasonal){
    ...
    # Female chicks (NB)
    A["ChF_NB", "2yF_BN"] <- pRep * mean.CS * S_C * gamma
    A["ChF_NB", "3yF_BN"] <- pRep * mean.CS * S_C * gamma
    
    # Male chicks (NB)
    A["ChM_NB", "2yF_BN"] <- pRep * mean.CS * S_C * (1-gamma)
    A["ChM_NB", "3yF_BN"] <- pRep * mean.CS * S_C * (1-gamma)
    
    # Female juvenile (BN)
    A["JuF_BN", "ChF_NB"] <- sF_NB[1]
    
    # Male juvenile (BN)
    A["JuM_BN", "ChM_NB"] <- sM_NB[1]
    
    # Female yearling (NB)
    A["1yF_NB", "JuF_BN"] <- sF_BN[1]
    
    # Male yearling (NB)
    A["1yM_NB", "JuM_BN"] <- sM_BN[1]
    
    # Female yearling (BN)
    A["1yF_BN", "1yF_NB"] <- sF_NB[2]
    
    # Male yearling (BN)
    A["1yM_BN", "1yM_NB"] <- sM_NB[2]
    
    # Female 2-yr (NB)
    A["2yF_NB", "1yF_BN"] <- sF_BN[2]
    
    # Male 2-yr (NB)
    A["2yM_NB", "1yM_BN"] <- sM_BN[2]
    
    # Female 2-yr (BN)
    A["2yF_BN", "2yF_NB"] <- sF_NB[3]
    
    # Male 2-yr (BN)
    A["2yM_BN", "2yM_NB"] <- sM_NB[3]
    
    # Female 3-yr (NB)
    A["3yF_NB", "2yF_BN"] <- sF_BN[3]
    A["3yF_NB", "3yF_BN"] <- sF_BN[4]
    
    # Male 3-yr (NB)
    A["3yM_NB", "2yM_BN"] <- sM_BN[3]
    A["3yM_NB", "3yM_BN"] <- sM_BN[4]
    
    # Females 3-yr (BN)
    A["3yF_BN", "3yF_NB"] <- sF_NB[4]
    
    # Males 3-yr (BN)
    A["3yM_BN", "3yM_NB"] <- sM_NB[4]
    ...
    # Female chicks (NB)
    A["ChF", "1yF"] <- sF_NB[2] * pRep * mean.CS * S_C * gamma
    A["ChF", "2yF"] <- sF_NB[3] * pRep * mean.CS * S_C * gamma
    A["ChF", "3yF"] <- sF_NB[4] * pRep * mean.CS * S_C * gamma
    
    # Male chicks (NB)
    A["ChM", "1yF"] <- sF_NB[2] * pRep * mean.CS * S_C * (1-gamma)
